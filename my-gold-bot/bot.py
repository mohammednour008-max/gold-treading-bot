...
# تحميل النموذج الذكي (تأكد أن الملف موجود في نفس المجلد)
...
def get_live_data():
    ...
    # إضافة المؤشرات الفنية (نفس المؤشرات المستخدمة في التدريب)
    df.ta.rsi(length=14, append=True)
    df.ta.sma(length=20, append=True)
    df.ta.macd(append=True)
    
    # تنظيف البيانات لتكون جاهزة للنموذج
